...
NIL_THREAD(ThreadTap, arg) { 
  ...
  while(true){  
    ...
    if ((getParameter(PARAM_PH) > (getParameter(PARAM_TARGET_PH)+PH_TOLERANCE)) && 
      ((now()-previous_ph_adjust)>=PH_ADJUST_DELAY) && (ph_state==CLOSE)) {
    ...
    else if( (previous_ph_adjust>=PH_OPENING_TIME) && (ph_state==OPEN))
    {
      ...
      if ((getParameter(PARAM_PH) < (getParameter(PARAM_TARGET_PH)-PH_TOLERANCE) ) && ((now()-previous_ph_adjust)>=PH_ADJUST_DELAY) & (ph_state==CLOSE))
    {
