...
NIL_WORKING_AREA(waThreadSensor, 70);      //check for memory allocation (enough // too much ?)
...
void getSensor() {
    ...
    for(char i=0;i<9;i++)
    {
      switch(Device[i].type)
      {
        ...
        // modify state vector depending on the conditions
